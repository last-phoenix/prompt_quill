...
class _LlamaContext:
    ...
    def load_session_file(self, path_session: str, n_token_capacity: int) -> List[int]:
        assert self.ctx is not None
        tokens_out = (llama_cpp.llama_token * n_token_capacity)()
        n_token_count_out = llama_cpp.c_size_t(0)
    ...
    def save_session_file(self, path_session: str, tokens: List[int]):
        assert self.ctx is not None
        n_tokens = len(tokens)
        c_tokens = (llama_cpp.llama_token * n_tokens)(*tokens)
